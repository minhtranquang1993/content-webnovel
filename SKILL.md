...
/content-webnovel blog20 review-short https://webnovel.vn/ai-bao-han-tu-tien/
...
Nếu user chỉ gửi URL + mô tả bằng lời ("viết bio cho truyện này", "làm bài review", "review ngắn / review đọc thử chương / review sâu cho truyện này", "top truyện xuyên không cho fbu.vn", "top truyện của Tối Bạch Đích Ô Nha", "bài hỏi đáp forum", "forum webnovel", "viết blog20 review", "blog20 review-short", "blog20 toplist", "keyword điền văn full", "giải thích thể loại tiên hiệp", "tiên hiệp là gì", "so sánh truyện A với truyện B", "truyện A hay truyện B hơn", "cẩm nang đọc ngôn tình cho người mới", "người mới nên bắt đầu từ đâu") → tự map sang type/subtype + tham số tương ứng. "review" trần → `review` (intro); "review ngắn / đọc chương / đọc thử / sâu" → `review-short`.
...
- File này **đồng bộ tự động** từ skill `/crawl-webnovel`: mỗi lần user chạy crawl, `crawl.py` ghi bản chính vào `~/Downloads/webnovel/truyen-data.json` **và copy 1 bản** vào `data/truyen-data.json` của skill này. Không cần sync tay.
...
## Archetype khung bài — chống "một màu" ở tầng HÌNH HÀI (deterministic theo hash)

Vấn đề gốc: title pool + 8 góc + verdict chỉ đa dạng **nội dung trong ô**; còn **trình tự section + loại block + giọng viết** thì mọi bài cùng subtype vẫn một khuôn (mở → info table → thân → verdict → "Giải đáp tò mò" → CTA). Đọc bài nào cũng cùng "vân tay". **Archetype** đa dạng hoá chính tầng hình hài đó, vẫn **stateless / tái tạo được** và **KHÔNG phá contract SEO/backlink**.

Archetype định **HÌNH HÀI** (thứ tự + block nào xuất hiện + đặt đâu + persona + họ title). 8 góc review vẫn định **NỘI DUNG 2-3 H2 giữa**. Hai thứ **xoay độc lập & cộng hưởng** (archetype × góc = đa dạng nhân đôi). Với review, góc chính vẫn chi phối câu mở + nội dung H2 thân; archetype quyết định *các H2 đó được sắp xếp/đóng khung thế nào* và *furniture quanh chúng*.

### Seed archetype (độc lập với góc / verdict / title)

Các hash đang dùng: góc `h mod len(nhóm)`, verdict `h mod 3`, title review `(h//3) mod 6`, title toplist `(h_top//3) mod 5`. Archetype dùng lát cắt **`// 7`** để **tách pha tương đối** khỏi title/góc/verdict (không dùng trực tiếp cùng residue `h mod 3` / `(h//3)`). Đây KHÔNG phải độc lập thống kê tuyệt đối — mọi seed vẫn là hàm deterministic của cùng `h`; mục tiêu chỉ là giảm đồng pha, chấp nhận được cho cơ chế prompt-only:

| Family | Seed | Công thức chọn archetype |
|---|---|---|
| review (intro) | `h` = Σ code-point `slug` truyện | `A = (h // 7) mod 4` |
| review-short | `h` như trên | `A = ((h // 7) + 1) mod 4` (lệch 1 so review intro cùng truyện → khác hình hài) |
| toplist / genre / guide | `h_top` = Σ code-point chuỗi target chuẩn hoá (thể loại strip "Truyện ", hoặc tên tác giả) | `A = (h_top // 7) mod 4` |
| versus | `h_vs` = Σ code-point của `slugA + slugB` **đã sort a→z** (đối xứng, không phụ thuộc thứ tự input) | `A = (h_vs // 7) mod 4` |
| forum | `h_fr` = Σ code-point `slug` (story) / target chuẩn hoá (category) | 3 post = archetype `s, (s+1)%4, (s+2)%4` với `s = (h_fr // 7) mod 4` |

**Announce archetype (NGOÀI block HTML, cùng chỗ dòng góc/title):**
```
Archetype: #[A] [tên] — persona: [giọng]
```

### Pool 4 archetype (dùng chung mọi family, hiện thực hoá theo từng subtype)

`[noun]` = "truyện"/"sách" theo Category-class. Persona pin theo từng archetype (4 giọng, 1 giọng/archetype): **#0 reviewer khách quan** / **#1 người đọc kể lại** (ngôi 1 tiết chế) / **#2 người dẫn-giải trực tiếp** / **#3 người tư vấn**. #1 dùng ngôi 1 thưa, tự nhiên (xem guardrail #8).

| # | Tên | Hình hài (thứ tự block) | Info-table | Block "Giải đáp tò mò" | Persona | Họ title |
|---|---|---|---|---|---|---|
| 0 | **Chuẩn** (classic) | mở → info-table → thân → verdict → H2 giải đáp → CTA | **giữ** vị trí hiện tại | H2 tail riêng | reviewer khách quan | Fam A = pool gốc (review 6 / toplist 5) |
| 1 | **Kể trải nghiệm** (narrative) | mở bằng **cảm nhận SUY RA TỪ SUMMARY/metadata** (KHÔNG dựng cảnh truyện; nếu nhắc "cảnh" chỉ dùng chi tiết đã có trong SUMMARY / text chương đã fetch) → thân kể liền mạch → **list "điều giữ chân người đọc"** → chốt cảm nhận | **ẩn** (thay bằng nhắc info inline trong câu); list nói trên thay vai trò table | **gộp 2-3 câu inline** trong thân, KHÔNG H2 tail | người đọc kể lại | Fam B (narrative) |
...
**Mọi công thức review/review-short BẮT BUỘC mở bằng `[Tên]`** (giữ primary keyword slot đầu — không được để "Đọc"/"Nên đọc"/"Mê…" đứng trước tên truyện):

| Arch | Họ | # | Công thức |
|---|---|---|---|
| 1 | narrative | 0 | `[Tên] – trải nghiệm đọc [noun] [thể loại] [năm]` |
| | | 1 | `[Tên]: vì sao cuốn người đọc [năm]` (non-fiction: `…được yêu thích [năm]`) |
| 2 | câu hỏi | 0 | `[Tên] có đáng đọc không? Giải đáp [năm]` |
| | | 1 | `[Tên] – nên đọc [noun] [thể loại] này không? [năm]` |
| 3 | khuyến nghị | 0 | `[Tên] – [noun] [thể loại] nên thử nếu bạn mê dòng này [năm]` |
| | | 1 | `[Tên] – gợi ý [noun] [thể loại] đáng đọc [năm]` |
...
| Arch | # | Công thức |
|---|---|---|
| 1 (narrative) | 0 | `Hành trình đọc [noun] [thể loại]: N gợi ý [năm]` |
| | 1 | `N [noun] [thể loại] đọc không rời tay [năm]` |
| 2 (câu hỏi) | 0 | `Đọc [noun] [thể loại] nào [năm]? N gợi ý đáng thử` |
| | 1 | `Hết [noun] để đọc? N [noun] [thể loại] nên đọc [năm]` |
| 3 (khuyến nghị) | 0 | `Mới đọc [thể loại]? Bắt đầu với N [noun] này [năm]` |
| | 1 | `N [noun] [thể loại] nên đọc trước tiên [năm]` |
...
8. Persona chỉ đổi **giọng/nhân xưng tiết chế**; KHÔNG chuyển thành quảng cáo lộ, KHÔNG "tôi" tràn lan phá giọng blogger đáng tin. Ngôi 1 dùng thưa, tự nhiên.
...
### Ảnh bìa ImgBB (review / review-short / toplist / genre / versus / guide)
...
|---|---|---|
| **review** / **review-short** | `Giải đáp tò mò về [noun] [Tên]` | `Thắc mắc thường gặp về [Tên]` · `Đọc [Tên]: những câu hỏi hay gặp` |
...
### Góc review (chống "một màu") — áp cho MỌI dạng review **intro**

Định nghĩa **một lần ở đây**; `blog20 review` và auto-switch pool=1 chỉ tham chiếu lại, KHÔNG mô tả lại.

> **Carve-out `review-short`:** cơ chế 8 Góc + guard "H2 thân giữa chỉ dựa SUMMARY, CẤM dựng tình tiết/nhân vật không có trong SUMMARY" (mục "Guard cứng theo góc" bên dưới) **CHỈ áp cho review intro** (`pbn review` / `blog20 review` / auto-switch pool=1). `review-short` KHÔNG kế thừa 8 Góc và KHÔNG bị guard SUMMARY-only ràng buộc phần thân giữa — nó có nguồn riêng (text chương đã đọc) + guard chương-sourced riêng (xem **LOẠI pbn → pbn review-short**). Title pool / category-class / freshness / backlink-unique vẫn áp cho cả hai.
...
**Chọn archetype trước** (`A = (h//7) mod 4`, xem "Archetype khung bài") → hình hài + persona + họ title. **Góc review** (chính+phụ) vẫn chi phối *nội dung* 2-3 H2 thân. Cấu trúc dưới là **archetype 0 (Chuẩn)** — coi là base; archetype 1-3 biến đổi hình hài theo bảng archetype (dời/ẩn info-table, đổi vị trí "Giải đáp tò mò", đổi câu mở theo persona), nhưng **giữ nguyên** mọi guardrail (khối "Nội dung truyện" lõi, backlink/self-link, verdict xoay `h mod 3`, freshness).
...
8. **Verdict xoay** theo `h mod 3` (điểm x/10 / verdict chữ / chốt cảm nhận) — *archetype 3 (verdict-first) dời verdict lên đầu bài*
9. `<h2>` theo pool "Giải đáp tò mò…" — 3-4 câu Q&A **đặc thù truyện này**, bám góc chính khi hợp — *archetype 1/2 hoà mục này vào thân (inline/Q&A-led), không H2 tail riêng*
10. CTA đọc full: **duy nhất 1** `<a href="{link truyện}">` về webnovel.vn

### pbn review-short (1 URL truyện — ĐỌC CHƯƠNG THẬT, chỉ fiction)

Biến thể review **đọc thật các chương free đầu** rồi phân tích phân cảnh/nhân vật/thoại cụ thể — sâu hơn `pbn review` (vốn chỉ dựa `SUMMARY`). **KHÔNG kế thừa 8 Góc + guard SUMMARY-only** (xem carve-out ở "Góc review"); chỉ mô tả phần **khác** so với `pbn review`, còn lại (block meta, self-link, backlink unique, ảnh ImgBB, category-class, freshness `[năm]`, mục "Giải đáp tò mò") **giữ y hệt `pbn review`**.
...
- **Exit code:** `0` OK (≥2 chương free — dùng để viết) · `2` URL sai/thiếu / thiếu Python / không tách được slug → báo + DỪNG · `3` chương 1 không đọc được (khóa/không tồn tại) → **announce + route `pbn review` intro** · `4` <2 chương free → **announce + route `pbn review` intro** (không đủ liệu phân tích, tránh bịa). **Mọi rc khác 0 → coi như lỗi scrape: báo + DỪNG (KHÔNG tự bịa nội dung chương).**
- **Chỉ dùng nội dung trong các block `<<<CHAP>>>` đã in.** Chương không xuất hiện = khóa/không đọc được → **KHÔNG nhắc, KHÔNG suy diễn**.
...
6. **Verdict "ấn tượng ban đầu"** — nhãn rõ **dựa trên các chương đầu đã đọc**; nếu có điểm số → ghi "ấn tượng ban đầu: x/10". **CẤM** claim/chấm điểm cho **toàn bộ tác phẩm** như đã đọc hết (đọc ~20/hàng nghìn chương).
7. `<h2>` "Giải đáp tò mò…" — pool H2 như `pbn review` (dòng review/review-short), 3–4 Q&A đặc thù truyện.
8. **CTA:** duy nhất **1** `<a href="{link truyện}">` về webnovel.vn (backlink unique như review).

**Guard chương-sourced (BẮT BUỘC — thay cho guard SUMMARY-only ở review intro):**
- Mọi chi tiết ở H2 thân giữa (5) **phải có trong text chương đã fetch**; **CẤM** suy diễn/thêm tình tiết ngoài text, **CẤM** spoiler chương chưa đọc, chương khóa/không đọc → không nhắc.
- Mọi nhận định + verdict phải neo khung "**qua các chương đầu**"; không kết luận toàn tác phẩm.
- Backlink/self-link giữ **nguyên contract review** (self-link đoạn mở + 1 backlink CTA; H2 phân tích không `<a>` webnovel.vn).
...
## LOẠI blog20 — HTML thuần, 1000-1500 chữ (review / review-short / toplist / genre / versus / guide)
...
Dùng toàn bộ logic `pbn review-short` (chạy `scrape-chapters.sh`, đọc chương free thật, title pool lệch index `((h//3)+1) mod 6` + neo trải-nghiệm-chương, 2–3 H2 phân tích chương-sourced, verdict "ấn tượng ban đầu", guard chương-sourced, cùng 4 exit code + routing non-fiction / chương-1-khóa / <2-free → **`blog20 review` intro**), chỉ áp 3 khác biệt blog20: **không `--site`, không block URL/Slug, không self-link** (đoạn mở dùng định nghĩa entity + TL;DR khung-chương, bỏ self-link). URL truyện xuất hiện đúng **1 lần** dưới dạng backlink ở CTA cuối; H2 phân tích chương vẫn **không** chèn `<a>` webnovel.vn nào. Dòng announce chương/route + title-pool giữ như `pbn review-short`.
...
### Archetype forum — 3 post = 3 HÌNH HÀI khác nhau (không chỉ khác hook)

`s = (h_fr // 7) mod 4` (`h_fr` = Σ code-point `slug` story / target chuẩn hoá category, xem "Archetype khung bài"). **3 post dùng archetype `s`, `(s+1) mod 4`, `(s+2) mod 4`** → 3 hình hài + 3 persona khác nhau, không chỉ đổi câu hỏi hook. Ánh xạ 4 archetype sang dạng post forum (plain text, không HTML/table):

| # | Tên | Hình hài post forum | Persona |
|---|---|---|---|
| 0 | Chuẩn | hook hỏi trực diện → giải đáp → mở rộng → chốt + CTA | reviewer khách quan |
| 1 | Kể trải nghiệm | hook cảm nhận cá nhân → kể lại trải nghiệm đọc liền mạch → chốt cảm nhận + CTA | người đọc kể lại (ngôi 1 tiết chế) |
| 2 | Hỏi–đáp sâu | hook = câu hỏi cụ thể → tách 2-3 khía cạnh trả lời từng cái → chốt + CTA | người dẫn-giải trực tiếp |
| 3 | Chốt trước | hook = khuyến nghị/kết luận ngay → biện giải vì sao → lưu ý + CTA | người tư vấn |

Announce (ngoài 3 post): `Archetype forum: post1 #[s], post2 #[(s+1)%4], post3 #[(s+2)%4]`. Guardrail bất biến forum giữ nguyên: mỗi post 1 URL trần CTA, không HTML/hashtag/bịa data, 500–1000 chữ, persona chỉ đổi giọng không dựng plot ngoài scrape.
...
### Quy tắc viết
- **3 post khác nhau rõ:** mỗi post 1 archetype (xem "Archetype forum") → khác **hình hài + persona + hook**, không chỉ paraphrase 1–2 câu.
- **KHÔNG hashtag.**
